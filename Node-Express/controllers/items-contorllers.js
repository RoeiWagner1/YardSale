...
const getItemsByUserId = async (req, res, next) => {
  ...
  if (!items || items.length === 0) {
    return next(
      new HttpError("Could not find an items for the provided user id.", 404)
    );
  }
...
const createItem = async (req, res, next) => {
  ...
  const { title, description, address, price, mobile, creator } = req.body;
...
const deleteItem = async (req, res, next) => {
  ...
  if (!item) {
    return next(new HttpError("Could not find an item for this id.", 404));
  }
